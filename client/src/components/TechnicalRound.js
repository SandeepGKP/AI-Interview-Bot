...
const TechnicalRound = ({ onComplete, roleTitle, candidateName, sessionId }) => {
  ...
  const fetchTechnicalQuestions = useCallback(async () => {
    ...
    try {
      ...
      setError(err.name === 'AbortError' ? t('request_timed_out_please_try_again_technical') : t('failed_to_load_technical_questions'));
      setQuestions([]);
    } finally {
      clearTimeout(timeoutId);
      setLoading(false);
    }
  }, [roleTitle, sessionId, t]);
    ...
    >
      <div className="pt-4 pb-4 px-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-purple-400">
            {t('technical_round_interview')}
          </h2>
        </div>
        {candidateName && <p className="text-lg text-gray-400 mb-4">{t('candidate')}: {candidateName}</p>}
      </div>

      {loading && <p className="text-center text-purple-400">{t('generating_technical_questions')}</p>}
      ...
      <div className="max-w-4xl mx-auto">
        {!loading && !error && questions.length > 0 ? (
          ...
          !loading && !error && <p className="text-center text-gray-500">{t('no_technical_questions_available')}</p>
        )}
      </div>
